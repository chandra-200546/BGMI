...
export function WeekendWarPage() {
  ...
  const prizeNumber = Number((activeTournament?.prize ?? "0").replace(/[^0-9]/g, "")) || 50000;
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          <div data-gsap-reveal className="clip-panel hud-panel p-6">
            ...
            <h3 className="mt-4 font-display text-4xl font-bold uppercase text-white">
              {activeTournament?.name ?? "Weekend War Season 4"}
            </h3>

            <div className="mt-6 grid grid-cols-2 gap-3">
              <HudMetric label="Total Prize" value={`₹${prizeNumber.toLocaleString("en-IN")}`} />
              <HudMetric label="Entry Fee" value={activeTournament?.fee ?? "Free"} />
              <HudMetric label="Format" value="Squad (TPP)" />
              <HudMetric label="Map Pool" value="Erangel / Miramar / Sanhok" />
            </div>
